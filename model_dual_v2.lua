...
-- this model will recover the dimension of the output the same as the input (not 4 times smaller)
local function ToolDetModelFull(input_channels, first_layer_channels, joint_num, compo_num)
    local m = nn.Sequential()
    -- conv1
    m:add(nn.SpatialConvolution(input_channels, first_layer_channels, 3, 3, 1, 1, 1, 1))
    m:add(nn.SpatialBatchNormalization(first_layer_channels))
    ...
    for i=1, 4 do
        ...
        local cat = nn.ConcatTable()
        cat:add(nn.Sequential():add(dual):add(nn.JoinTable(2))
                    :add(nn.SpatialConvolution(output_c, output_c, 1, 1, 1, 1))
                    :add(nn.SpatialBatchNormalization(output_c))
                    :add(nn.ReLU())
               )
        for j=3, i do
    ...
    for i=4, 3, -1 do
        ...
        cat:add(nn.Sequential()
                    :add(nn.NarrowTable(1,2))
                    :add(p):add(nn.JoinTable(2))
                    :add(nn.SpatialConvolution(input_c, output_c , 1,1,1,1))
                    :add(nn.SpatialBatchNormalization(output_c))
                    :add(nn.ReLU())
               )
        for j=3, i+1-2 do
    ...
    for i=2, 1, -1 do
        ...
        m:add(nn.Sequential():add(dual):add(nn.JoinTable(2))
                    :add(nn.SpatialConvolution(output_c, output_c, 1,1,1,1))
                    :add(nn.SpatialBatchNormalization(output_c))
                    :add(nn.ReLU())
             )
...
-- toolPartDetFull v=1: j_radius = 40, inputsize = 3, 384, 480], outputsize = [5+4, 384, 480], model_output_scale=1
